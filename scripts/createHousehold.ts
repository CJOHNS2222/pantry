...
async function main() {
  ...
  if (!email) {
    console.error('Usage: ts-node scripts/createHousehold.ts <email>');
    process.exit(1);
  }

  try {
    // Initialize Admin SDK with Application Default Credentials if available
    ...
    const householdData: any = {
      ...
      members: [
        { id: memberId, name: (uid ? email.split('@')[0] : email.split('@')[0]), email, role: 'Owner', status: 'Active' }
      ]
    };
