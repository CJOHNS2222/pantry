...
export const HouseholdActivityFeed: React.FC<HouseholdActivityFeedProps> = ({
        ...
        <div className="space-y-2">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="animate-pulse">
              <div className="h-4 bg-theme-primary/20 rounded mb-2"></div>
              <div className="h-3 bg-theme-primary/10 rounded w-3/4"></div>
            </div>
          ))}
        </div>
      </div>
  ...
  const getActivityIcon = (action: string) => {
    switch (action) {
      ...
      default:
        return <User className="w-4 h-4 text-theme-secondary opacity-50" />;
    }
  };
    ...
    <div className="bg-theme-secondary p-4 rounded-xl border border-theme">
      ...
      {activities.length > maxItems && (
        <div className="mt-3 pt-3 border-t border-theme">
          <p className="text-sm text-theme-secondary opacity-50 text-center">
